...
import properties from "./n11SettlementService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11SettlementServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Settlement Service",
    name: "n11SettlementServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Settlement Service",
    defaults: {
      name: "n11 Settlement Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/settlementService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11SettlementService.wsdl.xml"), {
      endpoint: endpoint
    });
