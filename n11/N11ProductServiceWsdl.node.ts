...
import properties from "./n11ProductService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11ProductServiceWsdl implements INodeType {
    description: INodeTypeDescription = {
        displayName: "n11 Product Service",
        name: "n11ProductServiceWsdl",
        group: ["transform"],
        version: 1,
        icon: "file:n11-logo.png",
        description: "Consume n11 Product Service",
        defaults: {
            name: "n11 Product Service",
            color: "#772244",
        },
        credentials: [
            {
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        let endpoint = "SERVER/productService/".replace("SERVER", server);
        let client = await soap.createClientAsync(path.join(__dirname, "n11ProductService.wsdl.xml"), {
            endpoint: endpoint
        });
