...
import properties from "./n11TicketService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11TicketServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Ticket Service",
    name: "n11TicketServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Ticket Service",
    defaults: {
      name: "n11 Ticket Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/ticketService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11TicketService.wsdl.xml"), {
      endpoint: endpoint
    });
