...
import properties from "./n11ShipmentCompanyService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11ShipmentCompanyServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Shipment Company Service",
    name: "n11ShipmentCompanyServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Shipment Company Service",
    defaults: {
      name: "n11 Shipment Company Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/shipmentCompanyService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11ShipmentCompanyService.wsdl.xml"), {
      endpoint: endpoint
    });
