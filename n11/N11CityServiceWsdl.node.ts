...
import properties from "./n11CityService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11CityServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 City Service",
    name: "n11CityServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 City Service",
    defaults: {
      name: "n11 City Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/cityService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11CityService.wsdl.xml"), {
      endpoint: endpoint
    });
