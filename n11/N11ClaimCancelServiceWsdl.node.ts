...
import properties from "./n11ClaimCancelService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11ClaimCancelServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Claim Cancel Service",
    name: "n11ClaimCancelServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Claim Cancel Service",
    defaults: {
      name: "n11 Claim Cancel Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/claimCancelService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11ClaimCancelService.wsdl.xml"), {
      endpoint: endpoint
    });
