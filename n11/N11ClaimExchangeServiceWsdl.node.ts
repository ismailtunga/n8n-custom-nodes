...
import properties from "./n11ClaimExchangeService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11ClaimExchangeServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Claim Exchange Service",
    name: "n11ClaimExchangeServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Claim Exchange Service",
    defaults: {
      name: "n11 Claim Exchange Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/claimExchangeService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11ClaimExchangeService.wsdl.xml"), {
      endpoint: endpoint
    });
