...
import properties from "./n11SapBankStatementEInvoiceService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11SapBankStatementEInvoiceServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Sap Bank Statement EInvoice Service",
    name: "n11SapBankStatementEInvoiceServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Sap Bank Statement EInvoice Service",
    defaults: {
      name: "n11 Sap Bank Statement EInvoice Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/sapBankStatementEInvoiceService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11SapBankStatementEInvoiceService.wsdl.xml"), {
      endpoint: endpoint
    });
