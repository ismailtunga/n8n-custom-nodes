...
import properties from "./n11SapCommissionEInvoiceDetailService.wsdl_properties";
const soap = require('soap');
const path = require("path");

export class N11SapCommissionEInvoiceDetailServiceWsdl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "n11 Sap Commission EInvoice Detail Service",
    name: "n11SapCommissionEInvoiceDetailServiceWsdl",
    group: ["transform"],
    version: 1,
    icon: "file:n11-logo.png",
    description: "Consume n11 Sap Commission EInvoice Detail Service",
    defaults: {
      name: "n11 Sap Commission EInvoice Detail Service",
      color: "#772244",
    },
    credentials: [
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/sapCommissionEInvoiceDetailService/".replace("SERVER", server);
    let client = await soap.createClientAsync(path.join(__dirname, "n11SapCommissionEInvoiceDetailService.wsdl.xml"), {
      endpoint: endpoint
    });
