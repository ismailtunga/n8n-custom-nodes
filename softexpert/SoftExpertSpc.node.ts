...
import properties from "./spc_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertSpc implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Spc",
    name: "softExpertSpc",
    group: ["transform"],
    version: 1,
    icon: "file:spc.png",
    description: "Consume SoftExpert Spc Service",
    defaults: {
      name: "SoftExpert Spc",
      color: "#772244",
	},
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/spc_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "spc.xml"),{
      endpoint: endpoint
    });
