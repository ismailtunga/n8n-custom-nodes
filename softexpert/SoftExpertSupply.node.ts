...
import properties from "./supply_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertSupply implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Supply",
    name: "softExpertSupply",
    group: ["transform"],
    version: 1,
    icon: "file:supply.png",
    description: "Consume SoftExpert Supply Service",
    defaults: {
      name: "SoftExpert Supply",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/sp_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "supply.xml"),{
      endpoint: endpoint
    });
