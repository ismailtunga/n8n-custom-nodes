...
import properties from "./calibration_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertCalibration implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Calibration",
    name: "softExpertCalibration",
    group: ["transform"],
    version: 1,
    icon: "file:calibration.png",
    description: "Consume SoftExpert Calibration Service",
    defaults: {
      name: "SoftExpert Calibration",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/cl_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "calibration.xml"),{
      endpoint: endpoint
    });
