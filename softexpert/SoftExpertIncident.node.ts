...
import properties from "./incident_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertIncident implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Incident",
    name: "softExpertIncident",
    group: ["transform"],
    version: 1,
    icon: "file:incident.png",
    description: "Consume SoftExpert Incident Service",
    defaults: {
      name: "SoftExpert Incident",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/in_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "incident.xml"),{
      endpoint: endpoint
    });
