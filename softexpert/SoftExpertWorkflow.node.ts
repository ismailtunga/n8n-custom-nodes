...
import properties from "./workflow_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertWorkflow implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Workflow",
    name: "softExpertWorkflow",
    group: ["transform"],
    version: 1,
    icon: "file:workflow.png",
    description: "Consume SoftExpert Workflow Service",
    defaults: {
      name: "SoftExpert Workflow",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/wf_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "workflow.xml"),{
      endpoint: endpoint
    });
