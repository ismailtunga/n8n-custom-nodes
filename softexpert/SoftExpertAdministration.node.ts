...
import properties from "./administration_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertAdministration implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Administration",
    name: "softExpertAdministration",
    group: ["transform"],
    version: 1,
    icon: "file:administration.png",
    description: "Consume SoftExpert Administration Service",
    defaults: {
      name: "SoftExpert Administration",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/adm_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "administration.xml"),{
      endpoint: endpoint
    });
