...
import properties from "./action_control_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertActionControl implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert ActionControl",
    name: "softExpertActionControl",
    group: ["transform"],
    version: 1,
    icon: "file:action_control.png",
    description: "Consume SoftExpert ActionControl Service",
    defaults: {
      name: "SoftExpert ActionControl",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/tmc_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "action_control.xml"),{
      endpoint: endpoint
    });
