...
import properties from "./risk_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertRisk implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Risk",
    name: "softExpertRisk",
    group: ["transform"],
    version: 1,
    icon: "file:risk.png",
    description: "Consume SoftExpert Risk Service",
    defaults: {
      name: "SoftExpert Risk",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/ri_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "risk.xml"),{
      endpoint: endpoint
    });
