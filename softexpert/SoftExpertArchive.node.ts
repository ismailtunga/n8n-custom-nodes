...
import properties from "./archive_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertArchive implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Archive",
    name: "softExpertArchive",
    group: ["transform"],
    version: 1,
    icon: "file:archive.png",
    description: "Consume SoftExpert Archive Service",
    defaults: {
      name: "SoftExpert Archive",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/arch_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "archive.xml"),{
      endpoint: endpoint
    });
