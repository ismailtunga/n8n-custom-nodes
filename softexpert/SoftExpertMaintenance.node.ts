...
import properties from "./maintenance_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertMaintenance implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Maintenance",
    name: "softExpertMaintenance",
    group: ["transform"],
    version: 1,
    icon: "file:maintenance.png",
    description: "Consume SoftExpert Maintenance Service",
    defaults: {
      name: "SoftExpert Maintenance",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/mn_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "maintenance.xml"),{
      endpoint: endpoint
    });
