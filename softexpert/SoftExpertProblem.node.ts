...
import properties from "./problem_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertProblem implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Problem",
    name: "softExpertProblem",
    group: ["transform"],
    version: 1,
    icon: "file:problem.png",
    description: "Consume SoftExpert Problem Service",
    defaults: {
      name: "SoftExpert Problem",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/pb_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "problem.xml"),{
      endpoint: endpoint
    });
