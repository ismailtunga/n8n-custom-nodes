...
import properties from "./document_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertDocument implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Document",
    name: "softExpertDocument",
    group: ["transform"],
    version: 1,
    icon: "file:document.png",
    description: "Consume SoftExpert Document Service",
    defaults: {
      name: "SoftExpert Document",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/dc_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "document.xml"),{
      endpoint: endpoint
    });
