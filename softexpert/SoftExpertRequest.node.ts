...
import properties from "./request_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertRequest implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Request",
    name: "softExpertRequest",
    group: ["transform"],
    version: 1,
    icon: "file:request.png",
    description: "Consume SoftExpert Request Service",
    defaults: {
      name: "SoftExpert Request",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/sr_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "request.xml"),{
      endpoint: endpoint
    });
