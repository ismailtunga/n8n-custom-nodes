...
import properties from "./training_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertTraining implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Training",
    name: "softExpertTraining",
    group: ["transform"],
    version: 1,
    icon: "file:training.png",
    description: "Consume SoftExpert Training Service",
    defaults: {
      name: "SoftExpert Training",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/tr_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "training.xml"),{
      endpoint: endpoint
    });
