...
import properties from "./project_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertProject implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Project",
    name: "softExpertProject",
    group: ["transform"],
    version: 1,
    icon: "file:project.png",
    description: "Consume SoftExpert Project Service",
    defaults: {
      name: "SoftExpert Project",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/pr_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "project.xml"),{
      endpoint: endpoint
    });
