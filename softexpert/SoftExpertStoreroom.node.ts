...
import properties from "./storeroom_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertStoreroom implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Storeroom",
    name: "softExpertStoreroom",
    group: ["transform"],
    version: 1,
    icon: "file:storeroom.png",
    description: "Consume SoftExpert Storeroom Service",
    defaults: {
      name: "SoftExpert Storeroom",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/stm_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "storeroom.xml"),{
      endpoint: endpoint
    });
