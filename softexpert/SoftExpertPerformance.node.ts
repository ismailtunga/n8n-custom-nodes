...
import properties from "./performance_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertPerformance implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Performance",
    name: "softExpertPerformance",
    group: ["transform"],
    version: 1,
    icon: "file:performance.png",
    description: "Consume SoftExpert Performance Service",
    defaults: {
      name: "SoftExpert Performance",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/st_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "performance.xml"),{
      endpoint: endpoint
    });
