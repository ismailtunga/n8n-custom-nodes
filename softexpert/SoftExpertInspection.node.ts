...
import properties from "./inspection_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertInspection implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Inspection",
    name: "softExpertInspection",
    group: ["transform"],
    version: 1,
    icon: "file:inspection.png",
    description: "Consume SoftExpert Inspection Service",
    defaults: {
      name: "SoftExpert Inspection",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/ip_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "inspection.xml"),{
      endpoint: endpoint
    });
