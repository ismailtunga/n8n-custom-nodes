...
import properties from "./pdm_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertPdm implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Pdm",
    name: "softExpertPdm",
    group: ["transform"],
    version: 1,
    icon: "file:pdm.png",
    description: "Consume SoftExpert Pdm Service",
    defaults: {
      name: "SoftExpert Pdm",
      color: "#772244",
	},
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/it_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "pdm.xml"),{
      endpoint: endpoint
    });
