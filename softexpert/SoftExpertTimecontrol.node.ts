...
import properties from "./timecontrol_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertTimecontrol implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Timecontrol",
    name: "softExpertTimecontrol",
    group: ["transform"],
    version: 1,
    icon: "file:timecontrol.png",
    description: "Consume SoftExpert Timecontrol Service",
    defaults: {
      name: "SoftExpert Timecontrol",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/wr_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "timecontrol.xml"),{
      endpoint: endpoint
    });
