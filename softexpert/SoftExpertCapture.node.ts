...
import properties from "./capture_properties" ;
const soap = require('soap');
const path = require("path"); 

export class SoftExpertCapture implements INodeType {
  description: INodeTypeDescription = {
    displayName: "SoftExpert Capture",
    name: "softExpertCapture",
    group: ["transform"],
    version: 1,
    icon: "file:capture.png",
    description: "Consume SoftExpert Capture Service",
    defaults: {
      name: "SoftExpert Capture",
      color: "#772244",
	},
	credentials: [
		{
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let endpoint = "SERVER/softexpert/webserviceproxy/se/ws/capt_ws.php".replace("SERVER",server);
    let client = await soap.createClientAsync(path.join(__dirname, "capture.xml"),{
      endpoint: endpoint
    });
